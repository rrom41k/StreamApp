...
public class AuthService : IAuthService
{
    ...
    public AuthService(string appSettingsToken, StreamPlatformDbContext dbContext, IHttpContextAccessor httpContextAccessor)
    {
    ...
    public async Task<ResultAuthDto> RegisterUser(AuthRegisterCommand authRegisterCommand, CancellationToken cancellationToken = default)
    {
        if (cancellationToken.IsCancellationRequested) // Проверка на отмену запроса
            throw new OperationCanceledException();
        
        CreatePasswordHash(authRegisterCommand.password, out byte[] passwordHash, out byte[] passwordSalt);
        User newUser = new(authRegisterCommand.email, passwordHash, passwordSalt);
        
        await _dbContext.Users.AddAsync(newUser, cancellationToken);
        await _dbContext.SaveChangesAsync(cancellationToken);
        
        IssueTokenPair(newUser, out string accessToken, out string refreshToken);

        return CreateResult(newUser, accessToken, refreshToken);
    }

    public async Task<ResultAuthDto> LoginUser(AuthLoginCommand authLoginCommandCommand, CancellationToken cancellationToken = default)
    {
        var user = await _dbContext.Users.FirstOrDefaultAsync(user => user.Email == authLoginCommandCommand.email, cancellationToken);
        
        if (!VerifyPasswordHash(authLoginCommandCommand.password, user.PasswordHash, user.PasswordSalt))
            throw new AuthenticationException("Wrong password!");

        IssueTokenPair(user, out string accessToken, out string refreshToken);
    ...
    public async Task<ResultAuthDto> GetNewTokens(CancellationToken cancellationToken = default)
    {
        ...
        if (result == null) throw new UnauthorizedAccessException("Invalid token or expired!");
        
        var user = await _dbContext.Users
            .FirstOrDefaultAsync(user => user.UserId == result.FindFirst("_id").Value, cancellationToken); // Ищем пользователя с _id указанным в токене
        ...
        IssueTokenPair(user, out string accessToken, out string newRefreshToken);
   ...
   private ResultAuthDto CreateResult(User user, string accessToken, string refreshToken)
   {
       ...
       return new ResultAuthDto(userDto, accessToken, refreshToken);
   }
    ...
    private string CreateToken(User user, DateTime expire)
    {
        ...
        {
            new Claim("_id", user.UserId),
            new Claim(ClaimTypes.Email, user.Email),
            new Claim(ClaimTypes.Role, user.IsAdmin ? "Admin" : "User"),
        };

        var key = new SymmetricSecurityKey(System.Text.Encoding.UTF8.GetBytes(_appSetTok));
    ...
    private ClaimsPrincipal ValidateToken(string? token)
    {
        ...
        var key = System.Text.Encoding.UTF8.GetBytes(_appSetTok);
