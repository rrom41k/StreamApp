...
public class UserService : IUserService
{
    ...
    public async Task<UserDto> CreateUser(UserCreateCommand user, CancellationToken cancellationToken = default)
    {
        ...
        User newUser = new(user.email, passwordHash, passwordSalt, user.isAdmin);

        if (_dbContext.Users.Contains(newUser)) throw new Exception("User with this email contains in DB");
        
        _dbContext.Users.Add(newUser);
        await _dbContext.SaveChangesAsync(cancellationToken);
        
        UserDto newUserDto = new UserDto(newUser.UserId, newUser.Email, newUser.IsAdmin);
    ...
    public async Task<UserDto> GetUserById(string id, CancellationToken cancellationToken = default)
    {
        ...
        UserDto existingUserDto = new UserDto(existingUser.UserId, existingUser.Email, existingUser.IsAdmin);
        
        return existingUserDto;
    }

    public async Task<UserDto> UpdateUser(string id, UserUpdateCommand updateUserData, 
        CancellationToken cancellationToken = default)
    {
        ...
        UserDto updatedUserDto = new UserDto(userToUpdate.UserId, userToUpdate.Email, userToUpdate.IsAdmin);
    ...
    public async Task<UserDto> DeleteUser(string id, CancellationToken cancellationToken = default)
    {
        ...
        UserDto removedUser = new UserDto(existingUser.UserId, existingUser.Email, existingUser.IsAdmin);
    ...
    {
        CreatePasswordHash(updateUserData.password, out byte[] passwordHash, out byte[] passwordSalt);
        user.Email = updateUserData.email ?? user.Email;
        user.PasswordHash = (updateUserData.password == null || updateUserData.password == "") ? user.PasswordHash : passwordHash;
        user.PasswordSalt = (updateUserData.password == null || updateUserData.password == "") ? user.PasswordSalt : passwordSalt;
